...
const Hero = ({ searchQuery }) => {
  ...
  const getplaceholder = (title) => {
    ...
    const initials = title.slice(0, 3).toUpperCase();
  ...
  if (allDummy) {
    return (
      <div className="min-h-screen bg-linear-to-b from-gray-900 to bg-gray-800 p-8 flex justify-center items-center">
          ...
          {paginatedBooks.map((book, index) => {
            ...
            return (
              <div
                id="hero"
                key={book.id}
                className="group relative bg-gray-800/30 rounded-xl md:rounded-2xl backdrop-blur-xl border border-white/10 hover:border-cyan400/30 transition-all duration-300 hover:-translate-y-1 md:hover:-translate-y-2 shadow-lg hover:shadow-xl md:shadow-2xl hover:shadow-cyan-400/10"
              >
                <a
                  href={book.infoLink}
                  target="_blank"
                ...
                <div className="p-4 md:p-6">
                  <div className=" relative aspect-4/5 w-full rounded-xl md:rounded-2xl overflow-hidden bg-linear-to-br from-cyan-400/10 to-blue-400/10">
                    <img
                    ...
                    />
                    <div className="absolute inset-0 bg-linear-to-t from bg-gray-900/30 via-transparent to-transparent" />
                    <div className="absolute bottom-0 left-0 right-0 p-2 md:p-4 bg-linear-to-t from-gray-900/90 to-transparent">
                  ...
                  <div className="mt-4 md:mt-6">
                    <h3 className="text-lg md:text-xl font-bold text-transparent bg-clip-text bg-linear-to-r from-cyan-400 to-blue-400">
                      {book.title}
                    </h3>
                    ...
                    <div className="flex items-center mt-2 md:mt-3">
                      <div className=" flex text-amber-400">
                        {[...Array(5)].map((_, i) => (
                          <StarIcon
                            key={i}
                            className={`h4 w-4 md:h-5 md:w-5 ${
                ...
                <div className="absolute inset-0 rounded-xl md:rounded-2xl bg-linear-to-br from-cyan-400/20 to-blue-500/20 opacity-0 group-hover:opacity-30 transition-opacity duration-300 pointer-events-none" />
              </div>
            );
          })}
        </div>
        <div className=" mt-6 md:mt-8 flex flex-wrap justify-center items-center gap-2 md:gap-3">
          ...
          {generatePageNumber().map((page, index) => (
            <button
              ...
              className={`px-3 py-1 md:px-4 md:py-2 rounded-md md:rounded-lg text-xs md:text-sm cursor-pointer ${
                page === currentPage
